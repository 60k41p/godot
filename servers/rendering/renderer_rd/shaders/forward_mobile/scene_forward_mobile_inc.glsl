...
layout(set = 0, binding = 18, std430) restrict readonly buffer InstanceUserdata {
	vec4 data[];
}
instance_userdata;
...
struct InstanceData {
	...
	uint instance_userdata_ofs; // Base offset in userdata buffer for per-instance user data.
